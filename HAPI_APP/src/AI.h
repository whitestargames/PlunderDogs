...
//Internal use
	...
	void attemptMove(const Map& map, std::shared_ptr<BattleEntity> currentShip, std::pair<const Tile*, eDirection> targetTile);
	void attemptShot(Battle& battle, const Map& map, std::shared_ptr<BattleEntity> firingShip);
//External use
	void handleMovementPhase(const Battle& battle, const Map& map, BattlePlayer& battlePlayer);
